...
class AccountDefaultHookSet(object):
    ...
    def send_invitation_email(self, to, ctx):
        subject = render_to_string("accounts/email/invite_user_subject.txt", ctx)
        message = render_to_string("accounts/email/invite_user.txt", ctx)
    ...
    def send_password_change_email(self, to, ctx):
        subject = render_to_string("accounts/email/password_change_subject.txt", ctx)
        subject = "".join(subject.splitlines())
        message = render_to_string("accounts/email/password_change.txt", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)

    def send_password_reset_email(self, to, ctx):
        subject = render_to_string("accounts/email/password_reset_subject.txt", ctx)
        subject = "".join(subject.splitlines())
        message = render_to_string("accounts/email/password_reset.txt", ctx)
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, to)
...
class HookProxy(object):
    ...
    def __getattr__(self, attr):
        return getattr(AccountDefaultHookSet, attr)
